...
\item Derived quantities
  \begin{description}
    \item[$\tensor{\sigma}$] Stress tensor
    \item[$\tensor{\epsilon}$] Strain tensor
    \item[$\zeta$] Variation of fluid content; variation of fluid volume per 
unit volume of 
porous material
...
We consider the displacement $\vec{u}$, volumetric strain $\epsilon_{v}$, 
and fluid pressure $p_f$ as unknowns,
\begin{align}
  \vec{s}^T &= (\vec{u}, \quad p_f, \quad \epsilon_{v})^T, \\
%
...
%
% fluid mass balance
  \frac{\partial \zeta(\epsilon_{v},p_f)}{\partial t} &= \gamma(\vec{x},t) - 
\nabla \cdot \vec{q}
(p_f) \text{ in }
\Omega, \\
%
\nabla \cdot \vec{u} &= \epsilon_{v} \text{ in } \Omega, \\
%
% Darcy's law
  \vec{q}(p_f) &= -\kappa (\nabla p_f - \vec{f}_f), \\
%
  \tensor{\sigma} \cdot \vec{n} &= \vec{\tau}(\vec{x},t) \text{ on }\Gamma_\tau, \\
%
  \zeta(\epsilon_{v},p_f) &= \alpha \epsilon_{v} + \frac{p_f}{M}, \\
%
  \vec{u} &= \vec{u}_0(\vec{x},t) \text{ on }\Gamma_u, \\
%
  \vec{q} \cdot \vec{n} &= q_0(\vec{x},t) \text{ on }\Gamma_q, \\
...
Using trial functions $\vec{\psi^u}_{trial}$, 
$\vec{\psi^{\epsilon_{v}}}_{trial}$, and $\vec{\psi^p}_{trial}$ and 
incorporating the Neumann boundary conditions, we write the weak form as \\
\begin{equation}
\vec{0} = \int_{\Omega} \vec{\psi}_{trial}^u \cdot \vec{f}(\vec{x},t) + 
\nabla \vec{\psi}_{trial}^u : -\sigma(\vec{u},p_f) \ d\Omega + 
\int_{\Gamma_{\tau}} \vec{\psi}_{trial}^u \cdot \vec{\tau}(\vec{x},t) \ d\Gamma,
\end{equation}
\begin{equation}
\int_{\Omega} \psi_{trial}^p \frac{\partial \zeta (\vec{u},p_f)}{\partial t} \ 
d\Omega = \int_{\Omega} \psi_{trial}^p \gamma(\vec{x},t) + \nabla \psi_{trial}^p 
. \vec{q}(p_f) \ d\Omega + \int_{\Gamma_q} \psi_{trial}^p (-q_0(\vec{x},t)) \ 
d\Gamma.
\end{equation}
\begin{equation}
\vec{0} = \int_{\Omega} \psi_{trial}^{\epsilon_{v}} \cdot (\nabla \cdot \vec{u} 
- \epsilon_{v}) \ 
d\Omega
\end{equation} \\

The structure of the code is such that the residual and jacobian functions 
relating solely to the governing equations are addressed separately from those 
involving the material rheology. For the reference of the developers, 
{\color{red} the labels underlying those residuals and jacobians dealing with 
the governing equations shall be colored RED}, and the {\color{green} the 
labels underlying those residuals and jacobians dealing with 
the material rheology shall be colored GREEN}.
...
(\frac{\partial}{\partial t}(\alpha \epsilon_{v} + \frac{p_f}{M})) \ d\Omega \\
&= \int_{\Omega} \psi_{trial}^p \eqnannotaterheo{\left(\alpha \frac{\partial 
\epsilon_{v}}{\partial t} + \frac{1}{M} \frac{\partial p_f}{\partial t} 
\right)}{f_0^p} \ 
d\Omega \\
%
...
\nabla \vec{\psi}_{trial}^u : \left(-\sigma(\vec{u},p_f) \right) \ 
d\Omega + \int_{\Gamma_{\tau}} \vec{\psi}_{trial}^u \cdot 
(\vec{\tau}(\vec{x},t)) \ d\Gamma \\
&= \int_{\Omega} \vec{\psi}_{trial}^u \cdot 
\eqnannotategovern{\left(\vec{f}(\vec{x},t) \right)}{g_0^u} + 
\nabla \vec{\psi}_{trial}^u : 
\eqnannotaterheo{\left(-(C:\varepsilon+\alpha p_f I)\right)}{g_1^u} \ 
d\Omega + \int_{\Gamma_{\tau}} \vec{\psi}_{trial}^u \cdot 
\eqnannotategovern{\left(\vec{\tau}(\vec{x},t) \right)}{g_0^u} \ 
d\Gamma \\
%
...
\eqnannotategovern{\left(\gamma(\vec{x},t)\right)}{g_0^p} + \nabla \cdot 
\psi_{trial}^p  \eqnannotaterheo{\left(-\kappa (\nabla p_f - \vec{f_f}) 
\right)}{g_1^p} \ d\Omega + \int_{\Gamma_q} \psi_{trial}^p 
\eqnannotategovern{ \left(-q_0(\vec{x},t) \right) }{g_0^p} \ d\Gamma \\
%
G^{\epsilon_{v}}(t,s) &= \int_{\Omega} \psi_{trial}^{\epsilon_{v}} \cdot 
\eqnannotategovern{\left(\nabla \cdot \vec{u} - \epsilon_{v} 
\right)}{g_{0}^{\epsilon_{v}}} 
\ d\Omega
\end{align} \\
...
F^u}{\partial \dot{p}} = 0 \\
%
J_F^{u \epsilon_{v}} &= \frac{\partial F^u}{\partial \epsilon_{v}} + t_{shift} 
\frac{\partial 
F^u}{\partial \dot{\epsilon_{v}}} = 0 \\
%
J_F^{\epsilon_{v}u} &= \frac{\partial F^{\epsilon_{v}}}{\partial u} + t_{shift} 
\frac{\partial F^{\epsilon_{v}}}{\partial \dot{u}} = 0 \\
%
J_F^{\epsilon_{v}p} &= \frac{\partial F^{\epsilon_{v}}}{\partial p} + t_{shift} 
\frac{\partial 
F^{\epsilon_{v}}}{\partial \dot{p}} = 0 \\
%
J_F^{\epsilon_{v}\epsilon_{v}} &= \frac{\partial F^e}{\partial e} + t_{shift} 
\frac{\partial 
F^{\epsilon_{v}}}{\partial \dot{\epsilon_{v}}} = 0 \\
%
...
\eqnannotaterheo{\left(t_{shift} \frac{1}{M}\right)}{J_{f0}^{pp}} 
\psi_{basis}^p \ 
d\Omega \\
%
J_F^{p\epsilon_{v}} &= \frac{\partial F^p}{\partial \epsilon_{v}} + t_{shift} 
\frac{\partial 
F^p}{\partial \dot{\epsilon_{v}}} = \int_{\Omega} \psi_{trial}^p t_{shift} 
\alpha 
\psi_{basis}^{\epsilon_{v}} \ d\Omega = \int_{\Omega} \psi_{trial}^p 
\eqnannotaterheo{\left(t_{shift} \alpha \right)}{J_{f0}^{p\epsilon_{v}}} 
\psi_{basis}^{\epsilon_{v}} \
d\Omega 
\end{align}

\begin{align}
J_G^{uu} &= \frac{\partial G^u}{\partial u} = \int_{\Omega} \nabla 
\vec{\psi}_{trial}^u : \frac{\partial}{\partial u} (- \sigma(\vec{u},p)) \ 
d\Omega = \int_{\Omega} \nabla \vec{\psi}_{trial}^u : 
\frac{\partial}{\partial u} (-(C:\varepsilon +\alpha p I)) \ d\Omega \\
&= \int_{\Omega} \nabla \vec{\psi}_{trial}^u : -C: \frac{1}{2} 
(\nabla + \nabla^T) \vec{\psi}_{basis}^u \ d\Omega = 
\int_{\Omega} \psi_{trial i,k}^u 
\eqnannotaterheo{\left(-C_{ikjl}\right)}{J_{g3}^{uu}} \psi_{basis j,l}^u \ 
d\Omega 
\\
%
J_G^{up} &= \frac{\partial G^u}{\partial p} = \int_{\Omega} \nabla 
\vec{\psi}_{trial}^u : \frac{\partial}{\partial p} (-\alpha p_f I) \ d\Omega = 
\int_{\Omega} \psi_{trial i,j}^u \eqnannotaterheo{\left(-\alpha 
\delta_{ij}\right)}{J_{g2}^{up}} \psi_{basis}^p \ d\Omega \\
%
J_G^{u\epsilon_{v}} &= \frac{\partial G^u}{\partial \epsilon_{v}} = 0 \\
%
J_G^{pu} &= \frac{\partial G^p}{\partial u} = 0 \\
%
J_G^{pp} &= \frac{\partial G^p}{\partial p} = \int_{\Omega} \nabla 
\psi_{trial}^p \cdot \frac{\partial}{\partial p} (-\kappa (\nabla p_f - 
\vec{f}) \ d\Omega = \int_{\Omega} \nabla \psi_{trial}^ p (- \kappa 
\nabla \cdot \psi_{basis}^p) \ d\Omega = \int_{\Omega} 
\psi_{trial,k}^p \eqnannotaterheo{\left(- \kappa 
...
\psi_{trial}^{\epsilon_{v}} 
\eqnannotategovern{\left(\delta_{ij}\right)}{J_{g1}^{\epsilon_{v}u}} 
\psi_{basis 
i,j}^u \ 
d\Omega\\
%
...
\eqnannotategovern{\left(-1\right)}{J_{g0}^{\epsilon_{v}\epsilon_{v}}} 
...
in a system of first order equations. Volumetric strain $\epsilon_{v}$ is 
neglected, as 
...
(\vec{u},p_f)}{\partial t} \right) d\Omega &=
 \int_{\Omega} \psi_{trial}^p 
\gamma(\vec{x},t) + \nabla \psi_{trial}^p . \vec{q}(p_f) \ d\Omega + 
...
Once again, the structure of the code is such that the residual and jacobian 
functions relating solely to the governing equations are addressed separately 
from those involving the material rheology. For the reference of the 
developers, {\color{red} the labels underlying those residuals and jacobians 
dealing with the governing equations shall be colored RED}, and the 
{\color{green} the labels underlying those residuals and jacobians dealing with 
the material rheology shall be colored GREEN}.\\

Identifying $F(t,s,\dot{s})$ and $G(t,s)$, we have

\begin{align}
  F^u(t,s,\dot{s}) &= \int_\Omega \trialvec[u] \cdot \eqnannotategovern{\left( 
\frac{\partial 
\vec{u}}{\partial t} 
\right)}{f_0^u} \, d\Omega \\
%
...
d\Omega \\
%
F^v(t,s,\dot{s}) &= \int_\Omega \trialvec[v] \cdot \eqnannotategovern{\left( 
\rho_{b} \frac{\partial 
\vec{v}}{\partial t} 
\right)}{f_0^v} \, d\Omega \\
%
G^u(t,s) &= \int_\Omega \trialvec[u] \cdot \eqnannotategovern{\vec{v}}{g_0^u} 
\, d\Omega \\
%
G^p(t,s) &= \int_{\Omega} \psi_{trial}^p (\gamma(\vec{x},t)) + \nabla 
\psi_{trial}^p \cdot (\vec{q}(p_f)) \ d\Omega + \int_{\Gamma_q} \psi_{trial}^p 
...
\eqnannotategovern{ \left(-q_0(\vec{x},t) \right) }{g_0^p} \ d\Gamma \\
%
G^v(t,s) &= \int_{\Omega} \vec{\psi}_{trial}^v \cdot (\vec{f}(\vec{x},t)) + 
\nabla \vec{\psi}_{trial}^v : \left(-\sigma(\vec{v},p_f) \right) \ 
d\Omega + \int_{\Gamma_{\tau}} \vec{\psi}_{trial}^v \cdot 
(\vec{\tau}(\vec{x},t)) \ d\Gamma \\
&= \int_{\Omega} \vec{\psi}_{trial}^v \cdot 
\eqnannotategovern{\left(\vec{f}(\vec{x},t) \right)}{g_0^v} + 
\nabla \vec{\psi}_{trial}^v : 
\eqnannotaterheo{\left(-(C:\varepsilon+\alpha p_f I)\right)}{g_1^v} \ 
d\Omega + \int_{\Gamma_{\tau}} \vec{\psi}_{trial}^v \cdot 
\eqnannotategovern{\left(\vec{\tau}(\vec{x},t) \right)}{g_0^v} \ 
d\Gamma 
\end{align}
...
\, 
\eqnannotategovern{t_\mathit{shift} \delta_{ij}}{J_{f0}^{uu}} \, 
\basisscalar[u]_j \, 
d\Omega \\
%
J_F^{up} &= \frac{\partial F^u}{\partial p} + t_{shift} \frac{\partial 
...
\int_\Omega 
\trialscalar[v]_i \, 
\eqnannotategovern{t_\mathit{shift} \, \rho_{b} \, \delta_{ij}}{J_{f0}^{vv}} \, 
\basisscalar[v]_j \, 
d\Omega \\
\end{align}
...
\basisvec[v] \, 
d\Omega = 
\int_\Omega \trialscalar[u]_i \, \eqnannotategovern{\delta_{ij}}{J_{g0}^{uv}} 
\, 
\basisscalar[v]_j 
\, d\Omega \\
%
...
%
J_G^{pp} &= \frac{\partial G^p}{\partial p} = \int_{\Omega} \nabla 
\psi_{trial}^p. \frac{\partial}{\partial p} (-\kappa (\nabla p_f - 
\vec{f}) \ d\Omega = \int_{\Omega} \nabla \psi_{trial}^ p (- \kappa 
\nabla \cdot \psi_{basis}^p) \ d\Omega = \int_{\Omega} 
\psi_{trial,k}^p \eqnannotaterheo{\left(-\kappa 
\delta_{kl}\right)}{J_{g3}^{pp}} 
...
d\Omega = \int_{\Omega} \nabla \vec{\psi}_{trial}^v : 
\frac{\partial}{\partial u} (-(C:\varepsilon +\alpha p I)) \ d\Omega \\
&= \int_{\Omega} \nabla \vec{\psi}_{trial}^v : -C: \frac{1}{2} 
(\nabla + \nabla^T) \vec{\psi}_{basis}^u \ d\Omega = 
\int_{\Omega} \psi_{trial i,k}^v 
\eqnannotaterheo{\left(-C_{ikjl}\right)}{J_{g3}^{vu}} \psi_{basis j,l}^u \ 
d\Omega 
\\
%
J_G^{vp} &= \frac{\partial G^v}{\partial p} = \int_{\Omega} \nabla 
\vec{\psi}_{trial}^v : \frac{\partial}{\partial p} (-\alpha p_f I) \ d\Omega = 
\int_{\Omega} \psi_{trial i,j}^v \eqnannotaterheo{\left(-\alpha 
