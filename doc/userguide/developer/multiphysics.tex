...
(-q_0(\vec{x},t)) \ d\Gamma \\
&= \int_{\Omega} \psi_{trial}^p 
\eqnannotate{\left(\gamma(\vec{x},t)\right)}{g_0^p} + \nabla \cdot 
\psi_{trial}^p  \eqnannotate{\left(-\kappa (\nabla p_f - \vec{f_f}) 
\right)}{g_1^p} \ d\Omega + \int_{\Gamma_q} \psi_{trial}^p 
...
\eqnannotate{\left(\vec{f}(\vec{x},t) \right)}{g_0^v} + 
\nabla \vec{\psi}_{trial}^v : 
\eqnannotate{\left(-(C:\varepsilon+\alpha p_f I)\right)}{g_1^v} \ 
d\Omega + \int_{\Gamma_{\tau}} \vec{\psi}_{trial}^v \cdot 
\eqnannotate{\left(\vec{\tau}(\vec{x},t) \right)}{g_0^v} \ 
d\Gamma 
\end{align}
